...
public class Person extends Point {
    ...
    public Person(City city, int x, int y, int maskState) {
        super(x, y);
        this.city = city;
...
//    public boolean wantLeave() {
//        return MathUtil.stdGaussian(sig, -0.99) > 0;
//    }

    private int state = State.NORMAL;

    public int maskState = MaskState.HAVE_MASK;
    ...
    private void action() {
        ...
        //位于室内聚集区，移动意愿降低
//        if (getX() > 200 && getX() <400 && getY() > 200 && getY()<400){
//            if (!wantLeave()){
//                return;
//            }
//        }else
    ...
    public void update() {
        ...
        if (state == State.CONFIRMED
                && Clock.worldTime - confirmedTime >= Constants.HOSPITAL_RECEIVE_TIME) {
        ...
        //处理病死者
        if ((state == State.CONFIRMED || state == State.FREEZE) && Clock.worldTime >= dieMoment && dieMoment > 0) {
        ...
        //处理发病的潜伏期感染者
        if (Clock.worldTime - infectedTime > stdRnShadowtime && state == State.SHADOW) {
            state = State.CONFIRMED;//潜伏者发病
            confirmedTime = Clock.worldTime;//刷新时间
        }
        //处理未隔离者的移动问题
        action();
        //处理健康人被感染的问题
        ...
        for (Person person : people) {
            ...
            if (maskState == MaskState.NO_MASK){
                if (random < Constants.BROAD_RATE && distance(person) < SAFE_DIST) {
                    this.beInfected();
                    break;
                }
            }else if (random < Constants.BROAD_RATE*(1-Constants.IMMUNE_RATE) && distance(person) < SAFE_DIST/2){
                this.beInfected();
                break;
            }
